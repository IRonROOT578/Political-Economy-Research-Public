...
四個主題可分開讀：政體設計（台灣／中國／俄羅斯／法國／南韓九套）、單一選區兩輪投票、一院議會制如何產生行政首長。
...
| 主題 | 讀什麼 |
|------|--------|
| [政體設計](topics/regime-designs/) | 台灣五夾＋中國一院議會制＋俄羅斯一院總統制＋法國一院議會制＋南韓一院總統制 |
| [政體設計提示詞](topics/regime-designs/prompts/政體設計/) | 定稿所依的提示詞（`0-基本`／`1-優化`／`配置`／`測試`／`惡搞`） |
| [兩輪兩兩對決](topics/two-round-pairwise-runoff/) | 單一選區、人工計票的兩輪選制與票樣 |
...
2. 台灣：正式總統制 **180＝100＋80、不用預提**；測試同層、用預提（含預提範例）；議會制某些議員＝單一 100；惡搞 225 FPTP。正式／測試另疊台灣優化與個人偏好；惡搞疊台灣基本。更新先淘汰再決選。
...
7. 選制：先看 [SNTV 兩輪套](topics/two-round-pairwise-runoff/outputs/README.md)（k＝3 與 k＝4 並列）。
8. 議會制行政：直接讀 [簡答](topics/unicameral-parliamentary-executive/outputs/簡答.md)。

## 授權

兩套授權，都**不是** ShareAlike：他人改作不會鎖住本庫後續更新。條件與標法見 [LICENSE](LICENSE)。

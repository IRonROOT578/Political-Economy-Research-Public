...
NINE = False  # 九選項頁面時設 True


def optno(m, key):
    """選項號次：依最喜歡的人分組連號；九選項每組「只選」在前。"""
    order = []
    for a in range(1, m + 1):
        if NINE:
            order.append(("只", a))
        order += [p for p in perms(m) if p[0] == a]
...
候選人以甲、乙、丙表示；選項號次依最喜歡的人分組連號：1 只選甲、2 `甲>乙>丙`、3 `甲>丙>乙`、4 只選乙、5 `乙>甲>丙`、6 `乙>丙>甲`、7 只選丙、8 `丙>甲>乙`、9 `丙>乙>甲`。
...
def nine():
    ...
    ex = [
        example(3, "例一：沒有人只選一人——和六選項一模一樣",
                {O(1, 2, 3): 40, O(1, 3, 2): 20, O(2, 1, 3): 15, O(2, 3, 1): 10, O(3, 1, 2): 10, O(3, 2, 1): 5}, 1,
                show_part=True,
                outro="九選項是六選項的延伸：沒有人圈「只選」（選項 1、4、7）時，結果與 [`../../k3-六選項/第二輪/票型分布例子.md`](../../k3-六選項/第二輪/票型分布例子.md) 例一相同。"),
        example(3, "例二：有人只選一人，仍有兩兩對決贏家",
                ...
                outro="選項 1（只選甲）的 15 票只進「甲對乙」「甲對丙」兩場；「乙對丙」那場它不表態，所以該場參與票數是 100－15＝85。"),
        example(3, "例三：強迫排完會被票面順序污染——九選項避開",
                {O(1, 3, 2): 10, O(2, 3, 1): 32, O(3, 2, 1): 28, B(1): 30}, 3, show_part=True,
                intro="有 30 人只在乎甲，對乙、丙沒有意見。九選項讓他們圈選項 1（只選甲）。"),
        example(3, "例三（續）：同一群人若被迫排完，照票面位置圈甲那一組的第一種順序 `甲>乙>丙`",
        ...
        example(3, "例四（續）：那 140 人改圈選項 1（只選甲）",
